...
const INPUT = '/data/.openclaw/workspace/hkjc_trainers_entries_links_2026-02-08.full.json';
const OUTPUT = '/data/.openclaw/workspace/hkjc_key_races_2026-02-08.json';
...
function extractHorseMeta(html) {
  ...
  const ratingStr = html.match(/現時評分[\s\S]{0,400}?>(\d+)<\/td>/i)?.[1] ?? html.match(/現時評分[\s\S]{0,400}?(\d{1,3})/i)?.[1];
...
function extractRaceTable(html) {
  // Find the first table that contains the header cell "頭馬<br />距離".
...
async function main() {
  ...
  const horseLinks = uniq(input.links.map(x => x.href).filter(h => h.includes('/local/information/horse?horseid=')));
  ...
  async function worker() {
    while (true) {
      ...
      try {
        ...
        // Key race rule (current): only need 1-2 margin <= 0.5, so we need horse to be 01 or 02 and margin <= 0.5.
