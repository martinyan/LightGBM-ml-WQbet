...
UA = 'openclaw-hkjc-lgbm-train-test/1.0'
...
def main():
    ...
    ap.add_argument('--out', default='hkjc_lgbm_noodds_train2325_test2526_q_qp.json')

    # LGBM overfitting resistance
    ap.add_argument('--numLeaves', type=int, default=31)
    ap.add_argument('--maxDepth', type=int, default=-1)
    ap.add_argument('--minDataInLeaf', type=int, default=200)
    ap.add_argument('--minGainToSplit', type=float, default=0.2)
    ap.add_argument('--lambdaL1', type=float, default=0.5)
    ap.add_argument('--lambdaL2', type=float, default=10.0)
    ap.add_argument('--featureFraction', type=float, default=0.85)
    ap.add_argument('--baggingFraction', type=float, default=0.85)
    ap.add_argument('--baggingFreq', type=int, default=1)
    ap.add_argument('--learningRate', type=float, default=0.05)
    ap.add_argument('--numBoostRound', type=int, default=5000)
    ap.add_argument('--earlyStoppingRounds', type=int, default=120)
    args = ap.parse_args()
    ...
    dfit = lgb.Dataset(X_fit, label=y_fit, feature_name=feat_keys)
    deval = lgb.Dataset(X_eval, label=y_eval, reference=dfit, feature_name=feat_keys)
    ...
    params = {
        ...
        'min_data_in_leaf': args.minDataInLeaf,
        'min_gain_to_split': args.minGainToSplit,
        'lambda_l1': args.lambdaL1,
        'lambda_l2': args.lambdaL2,
        'feature_fraction': args.featureFraction,
        'bagging_fraction': args.baggingFraction,
        'bagging_freq': args.baggingFreq,
        'verbosity': -1,
        'seed': 42,
        'num_threads': 4,
    }
    ...
    model = lgb.train(
        params,
        dfit,
        num_boost_round=args.numBoostRound,
        valid_sets=[dfit, deval],
        valid_names=['fit','eval'],
        callbacks=[lgb.early_stopping(args.earlyStoppingRounds, verbose=False)]
    )
    ...
    print(json.dumps({'out': args.out, 'races': len(per_race), 'best_iteration': model.best_iteration}, indent=2))
