...
UA = 'openclaw-hkjc-walkforward/1.0'
...
def main():
    ...
    ap.add_argument('--minTrainMeetings', type=int, default=30)
    ap.add_argument('--stepMeetings', type=int, default=10)
    ap.add_argument('--stake', type=float, default=10.0)
    ap.add_argument('--p12Threshold', type=float, default=None, help='optional: only bet races where (top1_p + top2_p) > threshold')
    ap.add_argument('--maxWorkers', type=int, default=24)
    ap.add_argument('--cache', default='hkjc_dividend_cache.json')
    ap.add_argument('--out', default='hkjc_walkforward_q_qp_compare.json')
    args = ap.parse_args()
    ...
    # walk-forward blocks (expanding train)
    blocks = []
    ...
    for bi, b in enumerate(blocks, 1):
        train_set = {(r,v) for r,v,_ in b['train']}
        test_set = {(r,v) for r,v,_ in b['test']}

        train_rows = [r for mk in train_set for r in by_meeting.get(mk, [])]
        test_rows = [r for mk in test_set for r in by_meeting.get(mk, [])]
        ...
        # Train models
        # XGB: small-ish params for speed/stability
        ...
        xgb_model = xgb.train(params, dtrain, num_boost_round=350)
        p_xgb = xgb_model.predict(xgb.DMatrix(X_test, feature_names=feat_keys))
        ...
        # group predictions by race and compute top2 pair
        def make_race_picks(preds):
            by_race = defaultdict(list)
            for r, pp in zip(test_rows, preds):
        ...
        picks_xgb = make_race_picks(p_xgb)
        picks_lr = make_race_picks(p_lr)

        # fetch dividends for both models' picks
        def eval_picks(picks):
            if args.p12Threshold is not None:
                picks = [p for p in picks if p.get('p12', 0.0) > args.p12Threshold]
        ...
        res_xgb = eval_picks(picks_xgb)
        res_lr = eval_picks(picks_lr)

        out_blocks.append({
            'block': bi,
            'train_meetings': len(train_set),
            'test_meetings': len(test_set),
            'test_start': b['test'][0][0],
            'test_end': b['test'][-1][0],
            'xgb': res_xgb,
            'logreg': res_lr,
        })
    ...
    # aggregate overall
    def sum_model(key):
        st_q=rt_q=st_qp=rt_qp=0.0
        races=0
        for b in out_blocks:
            m=b[key]
            races += m['races']
            st_q += m['Q']['stake']; rt_q += m['Q']['return']
            st_qp += m['QP']['stake']; rt_qp += m['QP']['return']
        return {
            'races': races,
            'Q': finalize(st_q, rt_q),
            'QP': finalize(st_qp, rt_qp),
            'combined': finalize(st_q+st_qp, rt_q+rt_qp)
        }
