...
UA = 'openclaw-hkjc-walkforward-tiercal/1.0'
...
def main():
    ...
    ap.add_argument('--stake10', type=float, default=10.0, help='dividends are per $10; keep this at 10')
    ap.add_argument('--tier50', type=float, default=0.80)
    ap.add_argument('--tier100', type=float, default=0.88)
    ap.add_argument('--tier300', type=float, default=0.92)
    ap.add_argument('--maxWorkers', type=int, default=24)
    ap.add_argument('--cache', default='hkjc_dividend_cache.json')
    ap.add_argument('--out', default='hkjc_walkforward_calibrated_tiered_q_qp_compare.json')
    args = ap.parse_args()
    ...
    def stake_for_p12(p12: float) -> float:
        if p12 > args.tier300:
            return 300.0
        if p12 > args.tier100:
            return 100.0
    ...
    def eval_tiered(picks):
        # prepare list of (pick, stake)
        items = [(p, stake_for_p12(float(p['p12']))) for p in picks]
        items = [(p, st) for p, st in items if st > 0]
        ...
        def fetch_one(p):
            return get_dividends(p['url'], p['pair'])

        with ThreadPoolExecutor(max_workers=args.maxWorkers) as ex:
            futs = [ex.submit(fetch_one, p) for p, _ in items]
            divs = []
        ...
        # divs order doesn't match items, so we refetch sequentially to align stakes (small set typically)
        # safer: do aligned fetch
        aligned = []
        for p, st in items:
            aligned.append((p, st, get_dividends(p['url'], p['pair'])))

        for p, st, d in aligned:
            # dividends are per $10
    ...
    while i + args.testMeetings <= len(meetings):
        blocks.append({'train': meetings[:i], 'test': meetings[i:i+args.testMeetings]})
        i += args.stepMeetings

    out_blocks = []

    for bi, b in enumerate(blocks, 1):
        train = b['train']; test = b['test']
        n_train = len(train)
        ...
        res_xgb = eval_tiered(picks_xgb)
        res_lr = eval_tiered(picks_lr)
        ...
        out_blocks.append({
            ...
            'tierThresholds': {'50': args.tier50, '100': args.tier100, '300': args.tier300},
            'xgb': res_xgb,
            'logreg': res_lr
        })
    ...
    def sum_model(key):
        ...
        return {'races_bet': races, 'combined': finalize(st, rt)}
