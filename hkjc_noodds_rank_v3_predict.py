#!/usr/bin/env python3
"""Predict NO-ODDS Top5 using ranker v3 (sectional + pace proxy).
...
Usage:
  python3 hkjc_noodds_rank_v3_predict.py --db hkjc.sqlite --in merged.json --out pred.json
"""
...
class FeatureBuilder:
    ...
    def build(self, racedate: str, venue: str, distance_m: int, class_num: int, surface: str, pick: Dict[str, Any]) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        # returns (features, style_aux)
        ...
        f = {
            ...
            # placeholders for race pace features (filled later)
            'field_size': None,
...
def main():
    ...
    ap.add_argument('--model', default='models/HKJC-ML_NOODDS_RANK_LGBM_v3_TOP1.txt')
    ap.add_argument('--meta', default='models/HKJC-ML_NOODDS_RANK_LGBM_v3_TOP1.infermeta.json')
    ...
    for f,a in zip(feats,auxs):
        f['field_size']=n
        f['pace_index_front_frac']=pace_index
        f['closer_x_pace']=(a.get('style_closer') or 0)*pace_index
        f['front_x_slow']=(a.get('style_front') or 0)*(1.0-pace_index)
    ...
    out={
        ...
        'pace': {'field_size': n, 'n_front': n_front, 'pace_index_front_frac': pace_index},
    }
    ...
    print(json.dumps({'ok': True, 'out': args.outPath}, ensure_ascii=False))
