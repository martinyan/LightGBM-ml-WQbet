...
UA = 'openclaw-hkjc-xgb-train-test/jockeyhash-1.0'
...
def jockey_hash(jockey: str) -> float:
    # stable pseudo-random in [0,1)
    if not jockey:
        return 0.0
    h = hashlib.sha1(jockey.encode('utf-8')).digest()
    # take 8 bytes -> int
...
def main():
    ...
    ap.add_argument('--stake', type=float, default=100.0, help='stake per pool per race')
    ap.add_argument('--maxWorkers', type=int, default=24)
    ap.add_argument('--cache', default='hkjc_dividend_cache.json')
    ap.add_argument('--out', default='hkjc_xgb_noodds_regv2_plusjockey_p12gt1p1.json')
    args = ap.parse_args()
    ...
    label_key = 'y_place'
    # keep only truly meta keys; we will now INCLUDE a jockey hash feature
    meta = {
    ...
    # ensure cur_win_odds still present in feat_keys (will be overridden to 0)
    # add derived jockey hash
    feat_keys2 = feat_keys + ['cur_jockey_hash']
    ...
    def vec(r):
        ...
        out.append(jockey_hash(r.get('cur_jockey') or ''))
        return out
    ...
    dfit = xgb.DMatrix(X_fit, label=y_fit, feature_names=feat_keys2)
    deval = xgb.DMatrix(X_eval, label=y_eval, feature_names=feat_keys2)
    ...
    p_test = model.predict(xgb.DMatrix(X_test, feature_names=feat_keys2))
    ...
    for (rd, venue, race_no), arr in by_race.items():
        ...
        picks.append({'racedate': rd, 'venue': venue, 'race_no': race_no, 'pair': pair, 'p1': p1, 'p2': p2, 'p12': p12, 'url': url})
    ...
    def get_div(url, pair):
        ...
        out = {'q_div': divs['Q'].get(pair), 'qp_div': divs['QP'].get(pair), 'qp_offered': divs['hasQP']}
        cache[k] = out
        return out
    ...
    with ThreadPoolExecutor(max_workers=args.maxWorkers) as ex:
        ...
        for fut in as_completed(futs):
            ...
            per_race.append({**p, 'q_div': d.get('q_div'), 'qp_div': d.get('qp_div'), 'qp_offered': bool(d.get('qp_offered'))})

    with open(args.cache, 'w', encoding='utf-8') as f:
        json.dump(cache, f)

    mult = args.stake/10.0
    stake_q = len(per_race)*args.stake
    ret_q = sum((r['q_div'] or 0.0)*mult for r in per_race)

    per_qp=[r for r in per_race if r.get('qp_offered')]
    stake_qp = len(per_qp)*args.stake
    ret_qp = sum((r['qp_div'] or 0.0)*mult for r in per_qp)

    out = {
        'model_base': 'HKJC-ML_XGB_NOODDS_REG_v2',
        'variant': 'PLUS_JOCKEY_HASH',
    ...
    print(json.dumps({'out': args.out, 'races_bet': len(per_race)}, indent=2))
