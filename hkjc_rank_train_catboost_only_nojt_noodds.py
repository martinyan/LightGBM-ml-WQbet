...
UA='openclaw-hkjc-catboost-only/1.0'
...
def main():
    ...
    ap.add_argument('--outDirModel', default='models/HKJC-ML_CB_ONLY_NOJT_NOODDS')
    ap.add_argument('--outDirReport', default='reports/HKJC-ML_CB_ONLY_NOJT_NOODDS')
    ...
    # feature selection: drop jockey, trainer, odds
    ...
    # remove any jockey/trainer rolling stats and odds
    feat_keys=[k for k in feat_keys if not k.startswith('jockey_') and not k.startswith('trainer_') and k!='cur_win_odds']

    # categorical cols remaining
    cat_cols=[c for c in ['cur_surface','venue'] if c in feat_keys]
    ...
    for (rd, venue, rn), arr in sorted(by_race.items()):
        ...
        top5=arr_sorted[:5]
        # p12_like: softmax concentration for top2
    ...
    # Q $10 flat every race (top2 pair) full fetch
    def q_job(ro):
        t=ro['top5']
        if len(t)<2:
            return None
    ...
    report={
        ...
        'model':'HKJC-ML_CB_ONLY_NOJT_NOTR_NOODDS',
        'data': args.data,
        'ranges': {
        ...
        'strategy': {'pool':'Q','stake_per_race': args.stake,'selection':'top2 pair', 'note':'CatBoostRanker only; no jockey/trainer; no odds feature'},
