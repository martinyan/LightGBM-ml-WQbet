...
def parse_singleT(html: str):
    ...
    for tr in trs:
        cells = [clean_cell(x) for x in re.findall(r'<t[dh][^>]*>([\s\S]*?)</t[dh]>', tr, flags=re.I)]
        if len(cells) >= 3 and cells[0] == '單T':
            return cells[1], (first_number(cells[2]) or 0.0)
    return None, 0.0
...
def main():
    ...
    ap.add_argument('--calibFracMeetings', type=float, default=0.2)
    ap.add_argument('--p12Threshold', type=float, default=1.35)
    ap.add_argument('--stake', type=float, default=100.0, help='stake per pool for Q/QP/SingleT')
    ap.add_argument('--stakeDivUnit', type=float, default=10.0, help='dividends are per this unit (HKJC=10)')
    ap.add_argument('--maxWorkers', type=int, default=24)
    ap.add_argument('--cache', default='hkjc_dividend_cache.json')
    ap.add_argument('--out', default='hkjc_rf_platt_noodds_p12gt1p35_Q100_QP100_SingleT100.json')
    ...
    ap.add_argument('--maxFeatures', default='sqrt')
    ...
    # split into fit vs calib by time
    n_train = len(train_meetings)
    n_cal = max(1, int(round(n_train * args.calibFracMeetings)))
    fit_meetings = train_meetings[:-n_cal]
    cal_meetings = train_meetings[-n_cal:]
    ...
    cal_rows = build_rows(cal_meetings)
    ...
    X_cal = np.asarray([vec(r) for r in cal_rows], dtype=np.float32)
    y_cal = np.asarray([int(r.get(label_key, 0) or 0) for r in cal_rows], dtype=np.int32)
    ...
    rf.fit(X_fit, y_fit)
    p_cal_raw = rf.predict_proba(X_cal)[:, 1]
    p_test_raw = rf.predict_proba(X_test)[:, 1]

    # Platt calibration: logistic regression on raw probability
    ...
    # make top2 per race on calibrated probs
    ...
    # p12 summary (before filtering)
    p12_all = [p['p12'] for p in picks]
    p12_stats = None
    if p12_all:
        s = sorted(p12_all)
        def q(frac):
            idx = int(round(frac * (len(s)-1)))
            return s[idx]
        p12_stats = {
            'n': len(s),
            'min': s[0],
    ...
    # filter by p12
    bets = [p for p in picks if p['p12'] > args.p12Threshold]
    ...
    def get_all(url):
        ...
        if k in cache:
            return cache[k]
        html = fetch_html(url)
        qqp = parse_dividends_Q_QP(html)
        st_combo, st_div = parse_singleT(html)
        out = {
            'q_div_map': qqp['Q'],
            'qp_div_map': qqp['QP'],
            'qp_offered': qqp['hasQP'],
            'singleT_combo': st_combo,
            'singleT_div': st_div
        }
        cache[k] = out
        return out
    ...
    stake = args.stake
    mult = stake / args.stakeDivUnit

    out_rows = []
    stake_q = stake * len(bets)
    stake_qp = 0.0
    stake_st = stake * len(bets)
    ret_q = 0.0
    ret_qp = 0.0
    ret_st = 0.0
    st_paid = 0
    ...
    with ThreadPoolExecutor(max_workers=args.maxWorkers) as ex:
        futs = {ex.submit(get_all, b['url']): b for b in bets}
        fetched = []
        for fut in as_completed(futs):
            b = futs[fut]
            try:
                fetched.append((b, fut.result()))
            except Exception:
                fetched.append((b, None))

    for b, d in sorted(fetched, key=lambda t:(t[0]['racedate'],t[0]['venue'],t[0]['race_no'])):
        if not d:
            continue
        q_div = d['q_div_map'].get(b['pair'])
        qp_div = d['qp_div_map'].get(b['pair'])
        qp_off = bool(d.get('qp_offered'))

        q_ret = (q_div or 0.0) * mult
        ret_q += q_ret

        qp_ret = 0.0
        if qp_off:
            stake_qp += stake
            qp_ret = (qp_div or 0.0) * mult
            ret_qp += qp_ret

        paid = (q_div is not None) or (qp_div is not None)
        st_combo = d.get('singleT_combo')
        st_div = float(d.get('singleT_div') or 0.0)
        st_ret = st_div if paid else 0.0  # $10 ticket payout only
        if paid:
            st_paid += 1
        ret_st += st_ret
        ...
        out_rows.append({
            **b,
            ...
            'qp_div': qp_div,
            'singleT_combo': st_combo,
            'singleT_div_per10': st_div,
            'singleT_paid': paid,
            'singleT_ret_assume10ticket': st_ret,
            'q_ret_on_100': q_ret,
            'qp_ret_on_100': qp_ret,
        })
    ...
    out = {
        'model': 'RF_NOODDS_PLATT',
        'trainRange': {'start': args.trainStart, 'end': args.trainEnd, 'meetings': len(train_meetings), 'fitMeetings': len(fit_meetings), 'calibMeetings': len(cal_meetings)},
        ...
        'p12_threshold': args.p12Threshold,
        'p12_stats_all_test_races': p12_stats,
        'stake_rule': 'For each bet race: $100 Q + $100 QP (if offered) + $100 SingleT. SingleT pays $10 ticket dividend when Top2 pair is in top3 (proxy: Q_div or QP_div exists).',
        'summary': {
            'Q': finalize(stake_q, ret_q),
            'QP': finalize(stake_qp, ret_qp),
            'SingleT': finalize(stake_st, ret_st),
            'combined': finalize(stake_q+stake_qp+stake_st, ret_q+ret_qp+ret_st)
        },
        'singleT_paid_races': st_paid,
        'rows': out_rows,
    ...
    print(json.dumps({'out': args.out, 'bets': len(out_rows)}, indent=2))
